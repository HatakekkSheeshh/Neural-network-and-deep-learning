...
protected:
    DLinkedList<Entry* >* table;  //array of DLinkedList objects: mỗi phần tử mảng là 1 dlinkedlist
...
V& xMap<K, V>::get(K key) {
    ...
    DLinkedList<Entry*>& list = table[index]; //get the list at the index
    //YOUR CODE IS HERE
    for(auto& pEntry: list){
...
bool xMap<K, V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)) {
    ...
    for(auto& pEntry: list){
        if(keyEQ(pEntry->key, key) && 
           (valueEqual == nullptr || valueEQ(pEntry->value, value))){
...
DLinkedList<K> xMap<K, V>::keys() {
    //YOUR CODE IS HERE
    DLinkedList<K> listK;//create list containing keys in table

    for(int idx = 0; idx < capacity; idx++){
        DLinkedList<Entry*>& list = table[idx];
        for(auto pEntry: list){
            listK.add(pEntry->key);
        }
    }
...
DLinkedList<V> xMap<K, V>::values() {
    //YOUR CODE IS HERE 
    DLinkedList<V> listVal;//create list containing values in table

    for(int idx = 0; idx < capacity; idx++){
        DLinkedList<Entry*>& list = table[idx];
        for(auto pEntry: list){
            listVal.add(pEntry->value);
        }
    }
...
string xMap<K,V>::toString(string (*key2str)(K&), string (*value2str)(V&)){
    ...
    for(int idx = 0; idx < this->capacity; idx++){
        ...
        for(auto& pEntry: list){
            ...
            if(key2str != nullptr) itemos << key2str(pEntry->key);
            else itemos << pEntry->key;
            itemos << ",";
            if(value2str != nullptr) itemos << value2str(pEntry->value);
...
void xMap<K,V>::ensureLoadFactor(int current_size){
    //cout << "capacity = " << capacity << "; load factor = " << loadFactor;
    int maxSize = (int)(loadFactor*capacity);
    
    //cout << "; ensureLoadFactor: count = " << count << "; maxSize = " << maxSize << "; current size = " << current_size << endl;
