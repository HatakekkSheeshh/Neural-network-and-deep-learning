...
xmap<string, TensorDataset<double, double>*>* DSFactory::get_datasets_3cc(){
    ...
    auto shape = train_table.shape();
    xt::svector<unsigned long, 4, std::allocator<unsigned long>, true> shape_vec(shape.begin(), shape.end());
    cout << shape2str(shape_vec) << endl;
    ...
    xt::xarray<double> T_train = onehot_enc(xt::cast<std::size_t>(t_train), 3);
    ...
    xt::xarray<double> T_valid = onehot_enc(xt::cast<std::size_t>(t_valid), 3);
    ...
    xt::xarray<double> T_test = onehot_enc(xt::cast<std::size_t>(t_test), 3);
