...
// g++ -Iinclude -Isrc -std=c++17 src/ann/layer/FCLayer.cpp
...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X) {
    ...
    m_aCached_X = X;
    double_tensor Y;
    if (X.dimension() == 1) Y = xt::linalg::dot(this->m_aWeights, X);
    else if(true) Y = xt::linalg::tensordot(X, xt::transpose(this->m_aWeights), {1}, {0}); 
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
    ...
    ++m_unSample_Counter;
    if (DY.shape()[0] == 1) {
        double_tensor WT = xt::transpose(m_aCached_X);
        m_aGrad_W = xt::linalg::outer(DY, WT);
        if (m_bUse_Bias) m_aGrad_b = DY ;
    }
    else{
        double_tensor DW = xt::zeros<double>({DY.shape()[0], m_aWeights.shape()[0], m_aWeights.shape()[1]});
        int nrows = DY.shape()[0];
        for(int r=0; r < nrows; r++){
            auto dy_r = xt::view(DY, r);
            auto x_r = xt::view(m_aCached_X, r);
            xt::view(DW, r) = xt::linalg::outer(dy_r,xt::transpose(x_r));
        }
        if (m_bUse_Bias) this->m_aGrad_b = xt::sum((DY), {0}) / m_unSample_Counter;
        m_aGrad_W += xt::sum((DW), {0}) / m_unSample_Counter;
    }
    double_tensor DX =  xt::linalg::tensordot(DY, m_aWeights, {1}, {0});
    return DX;
}
