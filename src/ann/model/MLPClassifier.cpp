...
double_tensor MLPClassifier::predict(
    ...
    for(auto batch: *pLoader) {
        ...
        double_tensor batch_pred = forward(X);
        
        if(first_batch) {
            auto shape = batch_pred.shape();
            shape[0] = 0;  // Set initial size to 0 for concatenation
            results = xt::zeros<double>(shape);
            first_batch = false;
        }
        
        results = xt::concatenate(xt::xtuple(results, batch_pred), 0);
        nsamples += static_cast<unsigned long long>(X.shape()[0]);
...
double_tensor MLPClassifier::forward(double_tensor X){
    //YOUR CODE IS HERE
    //cout << "test mlp forward..." << endl;
    double_tensor output = X;
    for(auto pLayer: m_layers){
        output = pLayer->forward(output);
    }
    //cout << "test mlp forward done..." << endl;
    return output;
}
void MLPClassifier::backward(){
    //YOUR CODE IS HERE
    //cout << "check mlp backward..." << endl;
    double_tensor DY = m_pLossLayer->backward();
    for(auto it = m_layers.bbegin(); it != m_layers.bend(); ++it){
        DY = (*it)->backward(DY);
    }
    //cout << "check mlp backward done..." << endl;
}
...
bool MLPClassifier::save(string model_path){
    try{
        ...
        string arch_file = (fs::path(model_path) / fs::path(m_pConfig->get("arch_file", "arch.txt"))).string();
